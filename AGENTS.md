...
- `src/features`: business feature modules (`auth`, `events`, `checkout`, `orders`, `account`, `admin`, `super-admin`, `scanner`, `merch`, `notifications`, `home`, `chat`) — each with `components/` and a `lib/` of `api.ts` (server reads) + `actions.ts` (Server Action writes).
...
- `backend`: separate Node.js/Express + MySQL API implementing the v1 domain — see `BACKEND.md`. `backend/src/mcp` holds the local MCP server + tools the assistant's LLM (WhatsApp bot, Telegram bot, website chat) acts through.
