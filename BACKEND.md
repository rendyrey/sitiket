...
| Merch orders | `POST /merch-orders` (signed-in only; a multi-seller cart returns one order PER seller; body includes `shipping: [{sellerId, courierCode}]` — one courier choice per seller, re-priced server-side), `GET /merch-orders/mine`\|`selling`\|`:id`, `POST /merch-orders/:id/cancel` |
