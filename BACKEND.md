...
| Merch orders | `POST /merch-orders` (signed-in only; a multi-seller cart returns one order PER seller), `GET /merch-orders/mine`\|`selling`\|`:id`, `POST /merch-orders/:id/cancel` |
...
| Profile | `PATCH /auth/me` — phone + delivery address (merch checkout prerequisite) |
...
- **Signed-in buyers only, with a saved address.** No guest flow; checkout 409s with `PROFILE_INCOMPLETE` until the buyer's profile has `phone` + `address` (snapshotted onto the order as the shipping destination — sellers arrange delivery themselves).
