...
| Event sub-resources | `GET/POST/DELETE /events/:eventId/images`, `/staff`, `GET/POST/PATCH /events/:eventId/ticket-types`, `/promo-codes`, `GET /events/:eventId/orders`, `GET /events/:eventId/orders/export?startDate&endDate` (admin/super_admin — every ticket order for this one event in that inclusive date range, items included; unpaginated, backs that event's Excel sales report) |
| Orders | `POST /orders`, `POST /orders/:id/verify-guest-email`, `GET /orders/mine` (transaction history — rows include `eventName`/`eventSlug` and `items` with `ticketTypeName`)\|`:id`\|`:id/guest`, `POST /orders/:id/cancel`, `GET /orders/export?startDate&endDate` (admin/super_admin — every order across every event this admin owns in that inclusive date range, items included; unpaginated, backs the admin Excel sales report) |
...
| Tickets & check-in | `GET /tickets/mine` (each ticket carries event + organizer context: `eventName`/`eventSlug`/dates/venue/city/`organizerName` — "My tickets" groups by them), `GET /orders/:orderId/tickets`, `POST /check-ins/scan` |
| Refunds | `POST/GET /orders/:orderId/refund-requests`, `GET /refund-requests/mine`, `POST /refund-requests/:id/approve`\|`reject`\|`complete` |
| Merch categories | `GET /merch-categories` (public; super_admin may pass `includeInactive`/`withCounts`), `POST`/`PATCH /merch-categories/:id`, `DELETE /merch-categories/:id` (guarded — 409 `CATEGORY_IN_USE` while products reference it) |
| Products (seller) | `GET /products/mine`, `POST /products`, `GET/PATCH/DELETE /products/:id` (delete = soft), `PATCH /products/:id/status`, `PUT /products/:id/variants` (replaces the whole option/variant matrix), `POST /products/:id/images` (multipart `image`, max 10), `DELETE /products/:id/images/:imageId` |
| Merch storefront (public) | `GET /merch` (hybrid search: FULLTEXT + typo-tolerant fuzzy, plus embedding-based semantic candidates when `VOYAGE_API_KEY` is set; `category`/`minPrice`/`maxPrice`/`sortBy`, paginated), `GET /merch/:slug` |
| Merch orders | `POST /merch-orders` (signed-in only; a multi-seller cart returns one order PER seller; body includes `shipping: [{sellerId, courierCode}]` — one courier choice per seller, re-priced server-side — and optional `promoCodes: [{sellerId, code}]` — one code per seller, re-validated server-side), `GET /merch-orders/mine`\|`selling`\|`:id`, `POST /merch-orders/:id/cancel`, `GET /merch-orders/export?startDate&endDate` (admin/super_admin — every one of the seller's own orders in that inclusive date range, items included; unpaginated, backs the admin Excel sales report) |
| Merch payments | `GET /merch-orders/:orderId/payments/instructions`, `POST /merch-orders/:orderId/payments` (proof upload), `GET /merch-orders/:orderId/payments`, `POST /merch-order-payments/:id/approve`\|`reject` |
| Merch promo codes | `GET /merch-promo-codes` (seller's own codes), `POST /merch-promo-codes`, `PATCH /merch-promo-codes/:id` (seller or super_admin; no DELETE — soft `isActive` toggle), `POST /merch-promo-codes/validate` (any signed-in buyer — checkout preview returning the discount rule for `{sellerId, code}`). Seller-scoped analogue of the event-nested `/events/:eventId/promo-codes`. |
| Regions (signed-in) | `GET /regions/provinces`, `GET /regions/provinces/:code/regencies`, `GET /regions/regencies/:code/districts`, `GET /regions/districts/:code/villages` — Indonesian region lists proxied from api.co.id through the `region_cache` table (see § _Merch invariants_ → shipping) |
...
| Website chat | `POST /chat/messages` (optional auth, per-IP `chatLimiter`) — `{ chatId, text }` → `{ reply, attachments, signedIn }` from the same assistant as the WhatsApp bot (see § _Website chat_) |
...
- A **successful** gate scan emails the buyer a branded "you're checked in" confirmation (event, venue, ticket type, and the scan timestamp in `Asia/Jakarta`) — `services/notification-service.js` `notifyTicketCheckedIn`, called from `scanTicket` only when `result === "success"` (never on `duplicate`/`invalid`/`expired`). Routed through the event organizer's SMTP config, same as every other buyer-facing email.

## Merch invariants (implemented)

- **Split-per-seller checkout.** Payment is a manual transfer to each SELLER's own bank account/QRIS, so `POST /api/merch-orders` splits a multi-seller cart into one order per seller — created in ONE transaction, so any stock failure rolls the whole checkout back. The frontend warns the buyer with a confirmation modal before a multi-seller checkout.
- **Signed-in buyers only, with a saved address.** No guest flow; checkout 409s with `PROFILE_INCOMPLETE` until the buyer's profile has `phone` + `address` + `village_code` (snapshotted onto the order as the shipping destination).
- **Courier shipping via api.co.id.** Addresses are structured on the Indonesian region hierarchy (province → regency/city → district → village); the 10-digit **village code** is what the client submits (it carries the postal codes), while shipping quotes key on its 6-digit **district code** — the granularity the Cek Ongkir v2 API (`/courier/v2/rates`) prices on. The client only ever submits a village code — `services/regional-service.js` resolves the full hierarchy server-side (`INVALID_VILLAGE` otherwise), so region names are never free-typed. Sellers must set a **departure address** (`seller_shipping_origins`, one row per owner) before `POST /products` succeeds (409 `SHIPPING_ORIGIN_REQUIRED` — same gate pattern as `EMAIL_CONFIG_REQUIRED` on events); they may also whitelist couriers (`enabled_couriers` JSON, NULL = all) and buyers only see/pick whitelisted ones. Checkout quotes couriers per seller group (`POST /shipping/quotes`) from total weight (`products.weight_grams` × qty, billed per started kg, min 1kg — `gramsToBillableKg`); order creation re-prices the chosen `courier_code` server-side (never trusting a client price), snapshots courier/cost/estimation/weight onto the order, and `total_amount = subtotal + shipping_cost`. v2 quotes every courier service separately; the backend keeps the cheapest service per courier (priced `price + handling_fee`) so the buyer still picks one option per courier. Both vendor plans are credit-limited, so responses are DB-cached in time windows: region lists in `region_cache` (`REGION_CACHE_DAYS`, default 30 — near-static data) and courier quotes in `shipping_cost_cache` keyed on (origin district, destination district, kg) (`SHIPPING_COST_CACHE_HOURS`, default 24). A vendor failure degrades to the stale cached copy when one exists. `API_CO_ID_KEY` unset → region/shipping endpoints answer 501 `SHIPPING_NOT_CONFIGURED` while the rest of the app keeps working.
- **Variant matrix, Shopee-style.** Up to 3 option groups × 20 options; each combination (`product_variants`) carries its own price/stock. Prices/totals are always server-computed from the live rows; stock reservation is the same guarded atomic UPDATE as tickets (base stock on `products` when no variants exist). `PUT /products/:id/variants` replaces the whole config atomically; order items snapshot name/label/price and `variant_id` is SET NULL on replace, so history survives.
- **24-hour payment hold** (`MERCH_PAYMENT_HOLD_HOURS`, default 24) instead of the ticket 10-minute window; the same 60s sweep in `server.js` expires stale merch orders and releases stock. As with tickets, `payment_expires_at` — not the sweep — is the authority for rejecting late proofs (`MERCH_ORDER_EXPIRED`).
- **Same manual proof-review model as tickets** (`merch_order_payments`): buyer uploads proof ("I have paid"), seller (or super_admin) approves/rejects. `services/payment-method-service.js` `resolvePaymentOptionsForSeller` is the single source of truth for how a seller can be paid — unlike events there is no QRIS opt-in flag; a seller with a QRIS config always offers it for merch.
- **Seller-scoped promo codes.** Merch promo codes mirror the event-scoped ticket promo system (`promo_codes` → `merch_promo_codes`) but are keyed on `seller_id` — the merch "store" is the container, just as an event is for tickets. Because checkout splits a cart into one order per seller, a code discounts only its own seller's order; the buyer submits an optional code **per seller** (`promoCodes: [{sellerId, code}]`, alongside the per-seller `shipping`). Prices/totals stay server-authoritative: `merch-order-service.js` validates each code and pre-computes its discount from that seller's subtotal, then inside the checkout transaction consumes one use atomically (`merch-promo-codes-repository.js` `incrementUsage`, the same guarded `used_count + 1 <= max_uses` UPDATE as tickets — a code on its last use can't be double-redeemed) and snapshots `promo_code_id`/`discount_amount` onto the order, so `total_amount = subtotal_amount - discount_amount + shipping_cost`. `releaseOrder` gives the use back (`decrementUsage`) on cancel/expire. `calculateDiscount` (clamped to the subtotal) and `validateForOrder` live in `services/merch-promo-code-service.js` — a self-contained copy of the ticket logic, kept independent of the ticket domain. The buyer sees a live discount at checkout via `POST /merch-promo-codes/validate` (returns just the discount rule; the authoritative discount is recomputed at order creation).
- **Guarded category delete**: `DELETE /merch-categories/:id` 409s (`CATEGORY_IN_USE`) while any live product references it; the super-admin list can include per-category product counts (`withCounts`).
- **Hybrid search, semantic layer optional.** Catalog search always runs FULLTEXT relevance + typo-tolerant fuzzy matching; with an embeddings provider configured, `services/embedding-service.js` adds embedding-based semantic candidates OR-ed into the match set and ranked after keyword relevance. Two provider options (plain `fetch`, no SDK): `VOYAGE_API_KEY` (Voyage AI `voyage-3.5-lite`), or the `EMBEDDINGS_BASE_URL`/`EMBEDDINGS_API_KEY`/`EMBEDDINGS_MODEL` trio for any OpenAI-compatible `/embeddings` endpoint (takes precedence when all three are set; the model must be an embedding model — chat models can't produce embeddings, and chat-only gateways like Kilo currently serve no embedding models). Rows record which model produced them; switching providers marks everything stale and the sweep re-embeds. There is deliberately **no vector database** — vectors are JSON in MySQL and cosine similarity runs in Node, which is milliseconds at merch-catalog scale (revisit only past tens of thousands of products). An empty candidate set (no products yet, or mid provider-switch) short-circuits before the query-embedding call, so an idle store makes zero vendor requests. Vectors live in `product_embeddings` (JSON, cosine similarity computed in Node — no vector DB at this scale) and refresh pull-based: a 60s sweep in `server.js` re-embeds products whose `content_hash` (SHA-256 of `name\ndescription`, mirrored in SQL as `SHA2(CONCAT(...), 256)`) went stale, so product writes never block on the vendor. Any vendor error degrades silently to keyword-only — search can never 500 because of embeddings.
- **Notifications ride two channels.** Email (seller-facing mail on the platform SMTP; buyer-facing mail through the seller's organizer SMTP config) plus in-app `notifications` rows for the header bell — written fire-and-log by `services/web-notification-service.js`, never failing the state change they announce. Ticket flows push bell notifications too (order placed / proof submitted → organizer; approve/reject → signed-in buyer).
- **Admin/organizer application, pending notice.** Submitting an `admin_applications` row (`admin-application-service.js` `apply`) sends the applicant both a bell notification and an email — "your application is being reviewed", linking to `ADMIN_SUPPORT_WHATSAPP_URL` for a faster answer — alongside the existing every-super-admin "new application" email. Both the create path and the `notify:pending-admin-applications` one-off script (for applications submitted before this notice existed) call the same `notifyApplicationPending` helper.

## Admin sales report & packing labels
...
A Bahasa-Indonesia customer-service bot ("Mimin SiTIKET") on the SiTIKET WhatsApp Business number that sells tickets and merch end-to-end — the same flows as the web checkouts, through the same services. Business rules and user journeys: [docs/business/WHATSAPP_BOT.md](docs/business/WHATSAPP_BOT.md); Meta dashboard + env setup: [DEPLOYMENT.md](DEPLOYMENT.md) § _WhatsApp bot setup_.

- **Path of a message.** Meta → `sitiket.com/api/webhooks/whatsapp` (Next Route Handler, a verbatim relay, since nginx only routes `/uploads/` here) → `routes/whatsapp.js` (mounted before `express.json()`; verifies `X-Hub-Signature-256` over the raw bytes, acks 200 immediately) → `services/whatsapp-bot-service.js`. Text goes through the shared assistant engine (`services/assistant-service.js`: history, model ↔ MCP loop, flood guard; prompt in `services/assistant-prompts.js`, identical persona/merch flow on every channel); a **photo** is always treated as a payment proof and handled without the LLM: `services/chat-proof-service.js` (shared with Telegram) — `pickProofTarget` picks the sender's open ticket order or one of their account's open merch orders (a caption naming the 8-char order ref decides when there are several; otherwise the buyer is asked), then it's downloaded from Meta, compressed + stored under `proofs/tickets/` or `proofs/merch/` via `middleware/upload.js` `storeImage`, and passed to the matching `submitProof`.
- **Local MCP server.** The LLM can only act through `mcp/sitiket-mcp-server.js`, an in-process MCP server (`@modelcontextprotocol/sdk`, in-memory transport) built **per message** with the sender's identity baked in; the bot is its MCP client and bridges its tools to chat-completions function calls. Tools live in `mcp/sitiket-tools.js` (tickets, reviews), `mcp/merch-tools.js` (merch) and `mcp/web-tools.js` (the website's signed-in ticket variants), shared by WhatsApp, Telegram (§ _Telegram bot_) and the website (§ _Website chat_); a tool may declare `channels`, and the per-message server registers only the tools for that message's role **and** channel. They call the existing services — `createOrder` (guest checkout + email OTP), `verifyGuestOtp`, `getPaymentInstructions`, `reviewProof`, `adminApplicationService.approve`, and for merch `listCatalog`, `quoteCart`, `createOrders` (merch-order-service), `buildAddressFields` + `updateProfile`, merch `submitProof` — so pricing, stock, OTP, proof and authorization rules are exactly the web's. Events and prices are read live from MySQL on every call (no snapshot/RAG layer to go stale).
- **Who can do what.** The sender is Meta's `wa_id` from the signed webhook (unspoofable). `chat-identity-service.js` `resolveSender` (shared with Telegram) finds the active accounts whose `phone` is that number (`users-repository.js` `findActiveByWhatsappId`: separator-stripped SQL match, then an exact `utils/phone.js` check). Role = super_admin > admin > buyer. The same match gives the **account** merch orders and address updates act on: the staff account, else the single match; several plain accounts on one number are ambiguous, so none is used. The per-message MCP server **only registers the tools that role may use**, so an admin tool doesn't exist for a buyer however the model is prompted:
  | Role | Tools |
  | --- | --- |
  | everyone | list/detail events, `create_order`, `verify_email_code`, `get_payment_instructions`, `get_my_orders` (ticket orders tagged with their own `orders.whatsapp_wa_id` + merch orders of their account) |
  | everyone, merch (needs a phone-matched account) | `list_merch` (10 newest purchasable), `get_merch_details`, `send_merch_photos` (2 per call, `page` for more; R2 WebP → JPEG via `sharp`, sent as WhatsApp images), `get_my_account`, `search_region` + `update_my_address` (province → regency → district → village, api.co.id cache), `quote_merch_shipping`, `create_merch_order`, `get_merch_payment_instructions` |
...
- **QR tickets in the chat.** `reviewProof` (web dashboard or bot) calls `services/chat-ticket-delivery-service.js` for orders with a `whatsapp_wa_id` (or `telegram_user_id` — § _Telegram bot_): a confirmation text plus one QR PNG per ticket (`qrcode`, level M — same `qr_payload` the web ticket page and gate scanner use), uploaded to WhatsApp's media API. Fire-and-log, never blocks the approval; email delivery is unchanged.
- **LLM.** Chat completions on the OpenAI-compatible endpoint already configured for embeddings (`EMBEDDINGS_BASE_URL`/`EMBEDDINGS_API_KEY`), model `WHATSAPP_BOT_MODEL` (default `gpt-4o-mini` — must be a chat model; production: `gpt-6-luna` + `WHATSAPP_BOT_REASONING_EFFORT=none`, sent as `reasoning_effort` only when set, because that reasoning model accepts function tools on Chat Completions only with reasoning off). Same model for every channel. ≤6 tool rounds per message, user text capped at 1500 chars, 15 messages / 10 min per sender, then one notice and silence (`floodDecision`) — every reply is a paid WhatsApp message from 1 Oct 2026, so floods must not trigger replies or LLM calls.
...
- **Tests.** `services/whatsapp-bot-service.test.js` covers the approval guard, phone matching, history trimming, proof routing, the flood rule, per-channel prompts and the webhook signature check.

## Telegram bot

The same assistant on Telegram ([@sitiket_assistant_bot](https://t.me/sitiket_assistant_bot)). Business rules: [docs/business/WHATSAPP_BOT.md](docs/business/WHATSAPP_BOT.md) § _Telegram bot_.

- **Transport: long polling, no webhook.** `server.js` calls `startTelegramPolling()` (`services/telegram-bot-service.js`) when `TELEGRAM_BOT_TOKEN` and the assistant LLM are configured: a `getUpdates` loop (50 s long-poll, 5 s backoff on errors) in the API process — no public route, no Next relay, no nginx change; updates queue at Telegram during restarts. Single pm2 instance only (a second poller or a webhook on the same token gets 409). Bot API calls are plain `fetch` (`services/telegram-client.js`); replies are sent with `parse_mode: "HTML"` via `toTelegramHtml` (escapes everything, then `*bold*` → `<b>`), falling back to plain text if Telegram rejects the markup.
- **Identity.** Private chats only; the Telegram user id is the chat key (`tg:<id>`) and the bot identity for guest ticket orders (`orders.telegram_user_id`, via `BotIdentity` in `orders-repository.js` — the same mechanism as `whatsapp_wa_id`). A persistent reply keyboard offers Telegram's native **share contact** button until the user shares; `handleContact` accepts only the sender's own contact (`contact.user_id === from.id`) and stores the number in `telegram_contacts`. With a stored number, `chat-identity-service.js` `resolveSender` (shared with WhatsApp) gives role/staff/account; without one the user is a buyer with no account (`verifiedPhone` unset → `create_order` asks for `buyerPhone`, merch tools answer `NO_LINKED_ACCOUNT` with the share-number hint).
- **Shared with WhatsApp.** Guest ticket tools (`create_order` / `verify_email_code` / `get_payment_instructions` / `get_my_orders`, `channels: ["whatsapp", "telegram"]`), merch tools (`send_merch_photos` → `sendPhoto` as JPEG), photo proofs (`services/chat-proof-service.js` `attachPaymentProof` — photos and image documents), QR delivery on approval (`services/chat-ticket-delivery-service.js` `sendTicketsToBotChat`, which picks WhatsApp or Telegram from the order's bot identity), the flood guard and the prompt (`assistant-prompts.js` telegram section + a per-turn "phone shared?" line). `/start` is a static greeting (no LLM call).

## Website chat

The same assistant ("Mimin SiTIKET", same engine, prompt persona and MCP tools) as a floating chat on every page of the site (`src/features/chat`, see FRONTEND.md § _Auth & data flow_). Business rules: [docs/business/WHATSAPP_BOT.md](docs/business/WHATSAPP_BOT.md) § _Website chat_.

- **Path.** Chat widget → Server Action `sendChatMessageAction` (session cookie forwarded by `apiFetch`) → `POST /api/chat/messages` (`routes/chat.js`: `chatLimiter` 40 req / 10 min per IP, `optionalAuth`, `schemas/chat-schemas.js`) → `services/web-chat-service.js` → `runAssistantTurn` with `channel: "web"`.
- **Identity = the session, not a phone.** A signed-in user acts as their own account (re-read from the DB — the JWT role can be stale) and gets the admin/super_admin reviewer tools when that's their role, with the same approval guard. A guest can browse and ask; every account-bound tool answers `SIGN_IN_REQUIRED` and the widget shows a *Masuk* button (`/login?redirect=<page>`). History/flood key: `web:<userId>`, or `web-guest:<chatId>` (a browser-generated UUID).
- **What differs from WhatsApp** (all via tool `channels` + the channel section of the prompt): tickets use the site's **signed-in checkout** (`mcp/web-tools.js` `create_order` → `createOrder(requester)`, no email OTP; name/email/phone from the account, `PHONE_REQUIRED` only when the account has none); payment proofs are uploaded on the existing **order pages** (`orderUrl` → `/orders/:id`, `/merch-orders/:id`), not in chat; `send_merch_photos` returns the images as reply `attachments` (backend-relative `/uploads/…` paths the widget renders with `next/image`) instead of sending WhatsApp messages. Merch, address change, courier quotes and reviewer tools are the shared tools unchanged.
- **Cost.** No per-message fee — only the LLM tokens. The per-chat flood guard answers 429 (`CHAT_RATE_LIMITED`) past 15 messages / 10 min, and the per-IP limiter stops guests from dodging it by minting new chat ids.
...
- **Organizer contact block.** The ticket-delivery (order paid) and proof-rejected emails include a "Need help? Contact the organizer" panel with the event's `contact_person_name`/`phone`/`email`, so buyers can reach the seller directly (`notification-service.js` `organizerContactRows`).
