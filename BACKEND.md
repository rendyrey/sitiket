...
- **Who can do what.** The sender is Meta's `wa_id` from the signed webhook (unspoofable). `resolveSender` finds the active accounts whose `phone` is that number (`users-repository.js` `findActiveByWhatsappId`: separator-stripped SQL match, then an exact `utils/phone.js` check). Role = super_admin > admin > buyer. The same match gives the **account** merch orders and address updates act on: the staff account, else the single match; several plain accounts on one number are ambiguous, so none is used. The per-message MCP server **only registers the tools that role may use**, so an admin tool doesn't exist for a buyer however the model is prompted:
  ...
  | everyone, merch (needs a phone-matched account) | `list_merch` (10 newest purchasable), `get_merch_details`, `send_merch_photos` (R2 WebP → JPEG via `sharp`, sent as WhatsApp images), `get_my_account`, `search_region` + `update_my_address` (province → regency → district → village, api.co.id cache), `quote_merch_shipping`, `create_merch_order`, `get_merch_payment_instructions` |
