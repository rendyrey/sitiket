...
| Orders | `POST /orders`, `POST /orders/:id/verify-guest-email`, `GET /orders/mine` (transaction history — rows include `eventName`/`eventSlug` and `items` with `ticketTypeName`)\|`:id`\|`:id/guest`, `POST /orders/:id/cancel` |
