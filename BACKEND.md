...
| Shipping | `POST /shipping/quotes` (signed-in; per-seller courier options for a cart, priced seller-origin → buyer-village), `GET /shipping/couriers` (the known courier catalog for the seller's enable/disable checkboxes) |
...
- **Courier shipping via api.co.id.** Addresses are structured on the Indonesian region hierarchy (province → regency/city → district → village); the 10-digit **village code** is what the expedition API keys on. The client only ever submits a village code — `services/regional-service.js` resolves the full hierarchy server-side (`INVALID_VILLAGE` otherwise), so region names are never free-typed. Sellers must set a **departure address** (`seller_shipping_origins`, one row per owner) before `POST /products` succeeds (409 `SHIPPING_ORIGIN_REQUIRED` — same gate pattern as `EMAIL_CONFIG_REQUIRED` on events); they may also whitelist couriers (`enabled_couriers` JSON, NULL = all) and buyers only see/pick whitelisted ones. Checkout quotes couriers per seller group (`POST /shipping/quotes`) from total weight (`products.weight_grams` × qty, billed per started kg, min 1kg — `gramsToBillableKg`); order creation re-prices the chosen `courier_code` server-side (never trusting a client price), snapshots courier/cost/estimation/weight onto the order, and `total_amount = subtotal + shipping_cost`. Both vendor plans are credit-limited, so responses are DB-cached in time windows: region lists in `region_cache` (`REGION_CACHE_DAYS`, default 30 — near-static data) and courier quotes in `shipping_cost_cache` keyed on (origin village, destination village, kg) (`SHIPPING_COST_CACHE_HOURS`, default 24). A vendor failure degrades to the stale cached copy when one exists. `API_CO_ID_KEY` unset → region/shipping endpoints answer 501 `SHIPPING_NOT_CONFIGURED` while the rest of the app keeps working.
...
- **Excel sales report.** `GET /api/merch-orders/export` and `GET /api/orders/export` (both `startDate`/`endDate`, `YYYY-MM-DD`, inclusive) return every matching order — unpaginated, items attached — for the signed-in admin. The frontend's `GET /api/admin/reports/export` Route Handler (`src/app/api/admin/reports/export/route.ts`) calls both in parallel and streams back one `.xlsx` workbook (via `exceljs`) with a "Merch Orders" and a "Ticket Orders" sheet; the backend routes are the actual `admin`/`super_admin` gate, the Route Handler just relays the session cookie and turns JSON into a download. UI: `/dashboard/admin/reports`.
