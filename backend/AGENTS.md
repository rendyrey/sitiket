...
- WhatsApp bot (see `BACKEND.md` § _WhatsApp bot_, business rules in [../docs/business/WHATSAPP_BOT.md](../docs/business/WHATSAPP_BOT.md)): new bot capabilities are MCP tools in `src/mcp/sitiket-tools.js` that call existing services — never re-implement a business rule inside a tool. Give each tool the narrowest `roles`, take identity only from the server-built context (`waId`, `staff`), never from model arguments, and gate every state-changing reviewer action with `isApprovalTypedByUser`. Bot replies/fixed copy are Bahasa Indonesia only. Buyer-facing WhatsApp sends must never block or fail the action they announce (fire-and-log, as in `services/whatsapp-ticket-service.js`).
