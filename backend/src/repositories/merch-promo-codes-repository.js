...
const TABLE = "merch_promo_codes";

/** @param {string} sellerId */
export const listBySeller = (sellerId) => db(TABLE).where({ seller_id: sellerId }).orderBy("created_at", "desc");
 ...
 * @param {import("knex").Knex} [executor]
 */
export const findBySellerAndCode = (sellerId, code, executor = db) =>
  executor(TABLE).where({ seller_id: sellerId, code }).first();

/**
 * @param {{ sellerId: string, code: string, discountType: "percentage" | "fixed_amount", discountValue: number, maxUses: number, validFrom?: Date, validUntil?: Date }} input
 */
