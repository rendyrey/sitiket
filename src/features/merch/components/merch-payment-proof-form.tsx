...
import { submitMerchPaymentProofAction } from "../lib/actions";

type MerchPaymentProofFormProps = {
  orderId: string;
...
/**
 * The merch buyer's "I have paid" step — uploading the proof is what
 * notifies the seller (email + bell) that the money is on its way.
 */
export default function MerchPaymentProofForm({ hasBankTransfer, hasQris, orderId }: MerchPaymentProofFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    // try/finally: a throw must never strand the button on "Uploading…" — a
    ...
    try {
      ...
      const result = await submitMerchPaymentProofAction(orderId, formData);
      if (!result.ok) {
  ...
  return (
    <div className="border-2 border-ink bg-white p-5 sm:p-7">
      <span className="tag">Confirm you have paid</span>
      <p className="mt-3 text-xs text-black/45">
        Upload your transfer receipt — the seller gets notified instantly and reviews it to release your order.
      </p>
      ...
      >
        {submitting ? "Uploading…" : "I have paid — submit proof"}
      </button>
    </div>
  );
}
