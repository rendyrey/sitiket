...
| Telegram bot (same assistant, free per message) — guest ticket checkout, shared-contact account linking for merch/admin tools, photo proofs, QR tickets in chat; landing-page QR to open it — see [WHATSAPP_BOT.md](./WHATSAPP_BOT.md) §10 | ✅ |
...
- [WHATSAPP_BOT.md](./WHATSAPP_BOT.md) — the Mimin SiTIKET assistant (WhatsApp bot, Telegram bot, website chat): buyer, organizer and Super Admin journeys, how users are recognised per channel, safety rules.
