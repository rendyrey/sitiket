...
- `/merch/checkout`: profile address + per-seller order summary; each seller group shows its courier options (quoted live via `POST /api/shipping/quotes` from the seller's departure address to the buyer's village, cheapest preselected, only the seller's enabled couriers) plus an optional **promo-code field** (validated live via `POST /api/merch-promo-codes/validate`, seller-scoped — the discount applies to that seller's items only), with per-seller and grand totals = items − discount + shipping; a multi-seller cart shows a confirmation modal ("N sellers → N separate payments") before creating one order per seller
- `/merch-orders/[id]`: merch order status — 24h payment-window countdown (reuses the ticket countdown), the seller's bank/QRIS instructions, "I have paid" proof upload, delivery details incl. the chosen courier + estimation, and an items/discount/shipping/total breakdown
...
- `/dashboard/admin`, `/events/new`, `/events/[slug]` (+ `/images`, `/ticket-types`, `/promo-codes`, `/staff`, `/orders` — which also carries that event's own Excel sales report, ticket orders only, above the order-review table), `/bank-accounts`, `/qris`, `/email-settings`, `/refunds`
...
- **Website chat** (`features/chat`): `ChatWidget` is mounted once in the root layout (hidden on `/print/*` and the `/dashboard/scan` camera page). The client component talks to the backend only through the `sendChatMessageAction` Server Action; guests get a browser `chatId` (UUID, localStorage), signed-in users are identified by the session cookie. Visible history is kept per viewer in localStorage (`sitiket-chat-history:<userId|guest>`, capped at 40 bubbles) — a convenience only; the assistant's real context lives on the backend. Replies use WhatsApp-style formatting rendered by `chat-message-text.tsx` (`*bold*`, `- ` lists, bare URLs → links; no HTML injection); image `attachments` render with `next/image` via `toAssetUrl`. Phone: full-screen sheet with page scroll locked; `sm`+: docked 380px card. Launcher is ink with a lime rim so it reads on the lime `/events` hero, dark pages and white pages alike. Telegram hand-off: a header link next to close (icon-only, labelled "Telegram" only on the 480–639px phone sheet — in the 380px docked card the label would wrap the subtitle) and a "Lanjut di Telegram" card under the quick prompts, shown only before the first message; both open `TELEGRAM_BOT_URL` (`config/assistant.ts`) in a new tab. Copy is Bahasa Indonesia, matching the assistant.
- **WhatsApp webhook**: `app/api/webhooks/whatsapp/route.ts` is a pure relay: it forwards Meta's GET handshake and the raw POST bytes + `X-Hub-Signature-256` to the backend's `/api/whatsapp/webhook`, which owns verification and the bot (BACKEND.md § _WhatsApp bot_). It exists only because nginx sends nothing but `/uploads/` to the backend. Keep it byte-exact (`arrayBuffer`, no JSON parse) or the signature check fails. No WhatsApp env vars live in the frontend.
